	...
	Widget build(BuildContext context) {
		return new Scaffold(
			appBar: AppBar(
				...
				title: Text("Cantlin", style: Theme.of(context).textTheme.title,),
			),
			body: objectList(context),
		);
	}
...
Widget objectList(BuildContext context) {
	...
	List<Widget> listRows = new List();
	listRows.add(categoryHeader("Items", itemColor));
	addListCategory(listRows, items, context);
	listRows.add(categoryHeader("Food", foodColor));
	addListCategory(listRows, food, context);
	listRows.add(categoryHeader("Light Source", lightSourceColor));
	addListCategory(listRows, lightSource, context);
	listRows.add(categoryHeader("Materials", materialsColor));
	addListCategory(listRows, materials, context);
	listRows.add(categoryHeader("Building materials", buildingMaterialsColor));
	addListCategory(listRows, buildingMaterials, context);
	listRows.add(categoryHeader("Furniture", furnitureColor));
	addListCategory(listRows, furniture, context);
	listRows.add(categoryHeader("Crafting station", craftingStationColor));
	addListCategory(listRows, craftingStation, context);
	listRows.add(categoryHeader("Weapons/Tools", weaponToolColor));
	addListCategory(listRows, weaponTool, context);
	listRows.add(categoryHeader("Armour", armourColor));
	addListCategory(listRows, armour, context);
...
Widget categoryHeader(String title, Color color) {
	return Container(
		padding: EdgeInsets.symmetric(horizontal: 8.0, vertical: 8.0),
		decoration: BoxDecoration(color: color),
		child: Text(title, style: header,),
	);
}